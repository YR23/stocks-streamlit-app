...
# For weekly data, we use a subfolder "data/1w/"
PREFIX = 'data/1w/'
...
def update_symbol_weekly(symbol):
    key = f"{PREFIX}{symbol}.csv"  # S3 object key, e.g., data/1w/AAPL.csv
    ...
    # Fetch the last 2 years of weekly data
    df_new = yf.download(tickers=symbol, period="5y", interval="1wk")
    if df_new.empty:
        print(f"No new data available for {symbol}")
        return

    # Extract the latest row (latest week of data) and its timestamp
    ...
    if df_existing is not None:
        if (df_existing["Datetime"] == timestamp).any():
            print(f"No new weekly data for {symbol} (timestamp {timestamp} already exists).")
        else:
            ...
            print(f"Updated {symbol} weekly CSV with new data at {timestamp}.")
    else:
        latest_df.to_csv(local_filename, index=False)
        s3.upload_file(local_filename, BUCKET_NAME, key)
        print(f"Created new weekly CSV for {symbol} with data at {timestamp}.")
