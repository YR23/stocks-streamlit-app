...
BUCKET_NAME = os.environ.get("BUCKET_NAME", "stocks-streamlit")  # default bucket name
PREFIX = 'data/'  # Files will be stored under s3://stocks-streamlit/data/
...
def update_symbol(symbol):
    key = f"{PREFIX}{symbol}.csv"  # S3 object key, e.g., data/AAPL.csv
    ...
    # Fetch the latest hourly data from yfinance (using a 1-month period to have enough data)
    ...
    if df_existing is not None:
        if (df_existing["Datetime"] == timestamp).any():
            print(f"No new data to update for {symbol} (timestamp {timestamp} already exists).")
        else:
            ...
            print(f"Updated {symbol} CSV with new data at {timestamp}.")
    else:
        # Create a new CSV file if it doesn't exist
        latest_df.to_csv(local_filename, index=False)
        s3.upload_file(local_filename, BUCKET_NAME, key)
        print(f"Created new CSV for {symbol} with data at {timestamp}.")
